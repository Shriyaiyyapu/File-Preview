    ...
    , currentDirectory(QDir::currentPath())
{
    ...
    // Set preview flag for previewOpenGLWidget
    previewOpenGLWidget->setIsPreview(true);
    previewOpenGLWidget->setGridVisible(false);
    previewOpenGLWidget->setCoordinateSystemVisible(false);
    ...
    // Connect signals (removed duplicate connection for browseButton)
...
void MainWindow::on_fileSelectionChanged(const QItemSelection &selected, const QItemSelection &deselected)
{
    ...
    if (!selected.indexes().isEmpty()) {
        ...
        QString fileType = fileInfo.isDir() ? "Folder" : fileInfo.suffix().toUpper() + " File";
        ui->fileTypeValue->setText(fileType);

        QDateTime modified = fileInfo.lastModified();
        ui->fileDateValue->setText(modified.toString("yyyy-MM-dd hh:mm:ss"));

        // Update preview
        updatePreview(filePath);
    } else {
        // Clear file details if nothing is selected
        ui->fileNameValue->setText("");
        ui->fileSizeValue->setText("");
        ui->fileTypeValue->setText("");
        ui->fileDateValue->setText("");
...
{
    qDebug() << "Browse button clicked - Opening file dialog";
